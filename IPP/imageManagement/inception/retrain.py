...
tf.app.flags.DEFINE_string('image_dir', '/home/zxc/Documents/dataset/',
                           """Path to folders of labeled images.""")
tf.app.flags.DEFINE_string('output_graph', '/tmp/imagenet/output_graph.pb',
                           """Where to save the trained graph.""")
tf.app.flags.DEFINE_string('output_labels', '/tmp/imagenet/output_labels.txt',
                           """Where to save the trained graph's labels.""")
tf.app.flags.DEFINE_string('summaries_dir', '/tmp/retrain_logs',
                          """Where to save summary logs for TensorBoard.""")

# Details of the training configuration.
tf.app.flags.DEFINE_integer('how_many_training_steps', 10,
...
tf.app.flags.DEFINE_integer('train_batch_size', 200,
                            """How many images to train on at a time.""")
tf.app.flags.DEFINE_integer('test_batch_size', 800,
...
tf.app.flags.DEFINE_string('model_dir', settings.MODEL_ROOT,
                           """Path to classify_image_graph_def.pb, """
                           """imagenet_synset_to_human_label_map.txt, and """
                           """imagenet_2012_challenge_label_map_proto.pbtxt.""")
tf.app.flags.DEFINE_string(
    'bottleneck_dir', os.path.join(settings.MODEL_ROOT, 'bottleneck'),
...
def connect():
  ...
  connect.update_state(state='PROGRESS', meta={'process_percent': 5})
  ...
  image_dir_path = os.path.join(settings.MEDIA_ROOT, 'photos')
  image_lists = create_image_lists(image_dir_path, FLAGS.testing_percentage,
                                   FLAGS.validation_percentage)
  class_count = len(image_lists.keys())
  if class_count == 0:
    print('No valid folders of images found at ' + image_dir_path)
    return -1
  if class_count == 1:
    print('Only one valid folder of images found at ' + image_dir_path +
  ...
  if do_distort_images:
    ...
    # cached them on disk.
    cache_bottlenecks(sess, image_lists, image_dir_path, FLAGS.bottleneck_dir,
                      jpeg_data_tensor, bottleneck_tensor)
  ...
  sess.run(init)
  connect.update_state(state='PROGRESS', meta={'process_percent': 10})
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if do_distort_images:
      ...
      train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks(
          sess, image_lists, FLAGS.train_batch_size, 'training',
          FLAGS.bottleneck_dir, image_dir_path, jpeg_data_tensor,
          bottleneck_tensor)
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      if ((i % 100) == 0 or is_last_step) :
	      process_percent = math.floor((i * 100)/FLAGS.how_many_training_steps)
	      connect.update_state(state='PROGRESS', meta={'process_percent': process_percent})
  ...
  version_name = time.strftime("%Y_%m_%d-%H-%M-%S",time.localtime(time.time()))
  output_graph_path = os.path.join(settings.MODEL_ROOT, 'output_graph_'+ version_name + '.pb')
  output_labels_path = os.path.join(settings.MODEL_ROOT, 'output_label_'+ version_name + '.txt')
  with gfile.FastGFile(output_graph_path, 'wb') as f:
    f.write(output_graph_def.SerializeToString())
  with gfile.FastGFile(output_labels_path, 'w') as f:
    f.write('\n'.join(image_lists.keys()) + '\n')
  return output_labels_path, output_graph_path
