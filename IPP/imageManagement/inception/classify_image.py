  ...
  def __init__(self,
    ...
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(
          FLAGS.mod_dir, 'output_labels.txt')
...
#       FLAGS.mod_dir, 'output_graph.pb'), 'rb') as f:
...
def create_graph(model_file = None):
  ...
  # Creates graph from saved output_graph.pb.
  if not model_file:
    model_file = os.path.join(FLAGS.mod_dir, 'output_graph.pb')
...
def run_inference_on_image(image, model_file=None, map_file=None):
  """Runs inference on an image.
  ...
  # 'final_result:0' A tensor containing the prediction across your own output_graph.pb
    ...
    # Creates node ID --> English string lookup.
    node_lookup = NodeLookup(map_file)
...
def classify_image (context):
  return run_inference_on_image(context['url'], context['model'], context['map'])


def main(_):
  maybe_download_and_extract()
  #the test image need to be put in the {mod_dir}, default path is /tmp/imagenet.
  image = (FLAGS.image_file if FLAGS.image_file else
           os.path.join(FLAGS.mod_dir, 'cropped_panda.jpg'))
